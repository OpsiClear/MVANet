...
class InferenceEngine:
    ...
    def infer_image(self, image: Image.Image, use_tta: bool = False) -> Image.Image:
        """
        ...
        """
        # Preprocess
        img_tensor, metadata = self.model.preprocess(image)
    ...
    def _load_and_preprocess(self, image_path: Path) -> tuple[Image.Image, torch.Tensor, dict]:
        ...
        with io.open(str(image_path), "rb") as f:
            ...
            with Image.open(img_buffer) as image:
                ...
                img_tensor, metadata = self.model.preprocess(image)
                return image.copy(), img_tensor, metadata

    def _save_outputs(
        self, mask: Image.Image, original_image: Image.Image, overlay_path: Path, mask_path: Path
    ):
    ...
    @staticmethod
    def _create_overlay(original_image: Image.Image, mask: Image.Image) -> Image.Image:
